...
from agents.transportation_expense_agent import transportation_expense_agent, _get_transportation_expense_agent
...
class TestTransportationExpenseAgent:
    """交通費精算専門エージェントのテスト"""
    
    def test_transportation_expense_agent_tool_callable(self):
        """transportation_expense_agentツールが呼び出し可能かテスト"""
        # ツールとして呼び出し可能か確認
        assert callable(transportation_expense_agent)
    
    def test_transportation_expense_agent_initialization(self):
        """transportation_expense_agentの初期化テスト"""
        # テスト用のセッションIDで初期化
        test_session_id = "test_session_001"
        agent = _get_transportation_expense_agent(session_id=test_session_id)
        
        assert agent is not None
        # Strands Agentオブジェクトにはtoolsプロパティがないため、初期化のみ確認
    
    def test_transportation_expense_agent_has_correct_tools(self):
        """transportation_expense_agentが正しいツールを持っているかテスト"""
    ...
    def test_transportation_expense_agent_multiple_instances(self):
        """transportation_expense_agentの複数インスタンス作成テスト"""
        # 異なるセッションIDで初期化
        agent1 = _get_transportation_expense_agent(session_id="test_session_003")
        agent2 = _get_transportation_expense_agent(session_id="test_session_004")
...
class TestAgentIntegration:
    ...
    @patch('agents.transportation_expense_agent._get_transportation_expense_agent')
    def test_transportation_expense_agent_can_process_simple_query(self, mock_get_agent, mock_tool_context):
        """transportation_expense_agentが簡単なクエリを処理できるかテスト（モック使用）"""
        # モックエージェントの設定
        ...
        # テスト実行
        response = transportation_expense_agent("こんにちは", tool_context=mock_tool_context)
    ...
    def test_transportation_expense_agent_handles_missing_invocation_state(self):
        """transportation_expense_agentがinvocation_state不足を適切に処理するかテスト"""
        # invocation_stateが空のモックコンテキスト
        mock_context = Mock()
        mock_context.invocation_state = None
        
        response = transportation_expense_agent("テスト", tool_context=mock_context)
